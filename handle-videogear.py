...
@torch.no_grad()
def run(weights='handle_best.pt',  # model.pt path(s)
...
>>>>>>> 93df7d953aeb611096eadc23e20f8c73c5228320
                # Print results
                for c in det[:, -1].unique():
                    n = (det[:, -1] == c).sum()  # detections per class
                    s += f"{n} {chinese_names[int(c)]}, "  # add to string
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_img or save_crop or view_img:  # Add bbox to image
                        ...
                        if [chinese_names[int(cls)]]+cor not in temp_list_class_cor:
                            temp_list_class_cor.append([chinese_names[int(cls)]]+cor)
                #print(web_text1)
                if len(temp_list_class_cor)!=6:
                    continue
                temp_list_class_cor=sorted(temp_list_class_cor,key=lambda s:s[2])
                for i,j in enumerate(sorted(temp_list_class_cor[:3],key=lambda s:s[1])):
                    web_text1.append(f"sw{i+1}{j[0]}")
                for i,j in enumerate(sorted(temp_list_class_cor[3:6],key=lambda s:s[1])):   
                    web_text1.append(f"sw{i+4}{j[0]}")  
                web_text=",".join(map(str, web_text1))
                print(f'"辨識結束:"{web_text1}{localtime}({t2 - t1:.3f}s)')
                #break
                #print(web_text1)
                    #if web_frame == None:
                    #     raw_frame
